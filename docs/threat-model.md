...
| Run retention escape | A run path or `state.json` redirects purge selection or deletion | Tested at-rest containment for the real `.agentproof/runs` parent; direct run-directory and final-path symlinks are rejected; purge opens `state.json` no-follow, verifies it is regular, and bounds reads to 64 KiB | Active concurrent component replacement is not descriptor-relative and remains out of scope |
